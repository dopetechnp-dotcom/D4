  ...
  try {
    ...
    if (!assetsBucket) {
      const { error } = await supabase.storage.createBucket(ASSETS_BUCKET, {
        ...
        fileSizeLimit: 20971520 // 20MB limit (increased for larger files)
      })
...
export async function uploadAsset(
  ...
  try {
    // Validate file size before upload (increased limits)
    ...
    if (file.size > maxSize) {
      ...
      return { 
        success: false, 
        error: `File is too large (${fileSizeMB}MB). Maximum size is ${maxSizeMB}MB for ${file.type.startsWith('video/') ? 'videos' : 'images'}. Please compress the file or use a smaller one.` 
      }
    }
    ...
    // Use API route to upload file (bypasses RLS)
    ...
    if (!response.ok) {
      ...
      return { 
        success: false, 
        error: errorData.error || 'Upload failed. Please try again with a smaller file.' 
      }
    }
    ...
    if (result.compressionRatio) {
      console.log(`✅ Asset uploaded successfully with ${result.compressionRatio}% compression`)
    }
...
export async function getVideoUrl(): Promise<string> {
  ...
  // Fallback to a placeholder video or empty string to prevent 404 errors
  return ''
}
