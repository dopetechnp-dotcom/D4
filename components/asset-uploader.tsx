...
export function AssetUploader() {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (file.size > maxSize) {
        ...
        setUploadResult({ 
          success: false, 
          message: `File too large (${fileSizeMB}MB). Maximum size is ${maxSizeMB}MB. Please choose a smaller file.` 
        })
        return
      }
  ...
  const getFileTypeDescription = (type: string) => {
    switch (type) {
      case 'logo':
        return 'SVG, PNG, or JPEG files (recommended: SVG for logos, max 10MB)'
      case 'video':
        return 'MP4 or WebM files (max 20MB)'
      case 'image':
        return 'PNG or JPEG files (max 10MB)'
      default:
        return ''
    }
  }
